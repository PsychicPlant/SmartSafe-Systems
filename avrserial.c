...
int main(void)
{
  ...
  //	fd1 stands for file descriptor, and it is a pointer to the open file that we will be inspecting, reading and writing to. The integer that it returns is an index number of the open file process.
  ...
  */
  ...
  //		This opens the file with R/D permission & more, and returns an int value that will be stored in the fd1 variable.
  ...
  //		The tcgetattr function will get the terminal parameters of the file opened, described by fd1, and save it into a termios struct. This is where we will use our oldtio.
